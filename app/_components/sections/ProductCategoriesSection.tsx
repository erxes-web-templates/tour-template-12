...
import ecommerceQueries from "@/app/dashboard/projects/_graphql/ecommerce/queries";
...
const ProductCategoriesSection = ({ section }: { section: Section }) => {
  ...
  const { data, loading, error } = useQuery(ecommerceQueries.productCategories, {
    variables: {
      perPage: 500,
      sortField: "order",
      sortDirection: 1,
    ...
    return (
      <section className="py-16">
        <div className="container mx-auto max-w-5xl px-4">
          <EmptyState title="No categories selected" description="Add product categories in the editor to feature them here." />
        </div>
      </section>
    );
  }
    ...
    <section className="py-16">
      <div className="container mx-auto max-w-6xl px-4">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {(loading ? Array.from({ length: 6 }) : categories).map((category: ProductCategory, index) => {
            if (loading) {
              return (
              ...
              <Card key={category._id} className="overflow-hidden">
                <CardHeader className="p-0">
                  <div className="relative h-48 w-full overflow-hidden bg-muted">
                    {imageUrl ? (
                      <Image src={imageUrl} alt={category.name} fill className="object-cover transition-transform duration-300 hover:scale-105" />
                    ) : (
